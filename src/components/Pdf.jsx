...
const processInput = (input) => {
  ...
  numbers.forEach((number) => {
    if (number.includes("-")) {
      let [start, end] = number.split("-").map(Number);
...
const allocateRollNumber = (rollNumberList, experimentNumberList, experimentsPerStudent) => {
...
const allocateRollNumberRandomly = (rollNumberList, experimentNumberList, experimentsPerStudent) => {
  const allocatedRollNumbers = [];
  ...
  rollNumberList.forEach((rollNumber) => {
    const experiments = extendedExperimentList.splice(0, experimentsPerStudent);
    allocatedRollNumbers.push({ rollNumber, experiments });
  });
  return allocatedRollNumbers;
};
